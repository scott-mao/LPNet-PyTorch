...
#muti-scale coarse to fine network
#baseline total params: 152015(换成CA之后参数量不变)
...
class MSCFB(nn.Module):
    ...
    def forward(self, x):
        ...
        #xs = torch.chunk(output_1,4, dim=1)
...
class LPNet(nn.Module):
    def __init__(self, args):
        ...
        #self.quater_cat = conv_layer(in_channels*2,in_channels,1)
    ...
    def forward(self, x):
        ...
        x = self.head(x)
        res = x
        halfup = F.upsample(halfup, x.size()[2:], mode='bilinear')
